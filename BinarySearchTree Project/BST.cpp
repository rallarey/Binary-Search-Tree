...
bool BST::insert(string s){
	...
	if (root == NULL){
		...
		while (n != NULL){
			if (s < n->data->phrase){
				if (n->left == NULL){
					n->left = new TNode(s); // set n's left child to be new node with data 's'
				...
				if (n->right == NULL){
					n->right = new TNode(s); // set n's right child to be new node with data 's'
...
TNode *BST::find(string s){
	...
	while (tmp){
		if (tmp->data->phrase == s){ // just break out of loop if the phrase of tmp is the parameter
			break;
		} else if (s < tmp->data->phrase){ // if string 's' is less than roots data, tmp points to left child
				tmp = tmp->left;
		} else if (s > tmp->data->phrase){ // if string 's' is less than roots data, tmp points to right child
			tmp = tmp->right;
		}
	}
	return tmp;
...
TNode *BST::remove(string s){
	...
	if (tmp->left == NULL && tmp->right == NULL){ // no kids
		...
		Phrase *value = temp->data; // getting the phrase from temp
